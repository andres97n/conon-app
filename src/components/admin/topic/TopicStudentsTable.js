...
import { startLoadNewStudentsForTopics, startSaveStudentsToTopic } from '../../../actions/admin/topic';
import { getDataIdsForModel, setRangeNumberInput } from '../../../helpers/admin';

export const TopicStudentsTable = React.memo(({
    topic,
    toast
}) => {
    ...
    const handlePutStudents = () => {
        dispatch( startSaveStudentsToTopic(
    ...
    const setChangeAge = (event) => {
        setAge(event.value);
        if (topic) {
            dispatch( startLoadNewStudentsForTopics( topic.id, event.value ) );
        }
    }
