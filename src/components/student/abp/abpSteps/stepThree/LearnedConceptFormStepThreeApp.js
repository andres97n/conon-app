...
import { 
  startSaveLearnedConceptStepThreeAbp 
} from '../../../../../actions/student/abp_steps/learnedConceptStepThreeAbp';


export const LearnedConceptFormStepThreeApp = React.memo(({
  teamId,
  conceptsSize,
  toast
}) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      handleConfirmSaveLearnedConcepts( data );
    }
  });

  const { values, errors, setValues, resetForm, handleChange, handleSubmit } = formik;

  const handleSubmitLearnedConceptsAbp = ( data ) => {
    //Guardar Conceptos
    let newConcepts;
    if (data.concepts.length === 1) {
      newConcepts = {
        team_abp: teamId,
        learned_concept: data.concepts[0].concept,
        active: true
      };
    } else {
      newConcepts = data.concepts.map( data => ({
        team_abp: teamId,
        learned_concept: data.concept,
        active: true
      }));
    }
    dispatch( startSaveLearnedConceptStepThreeAbp( newConcepts, toast ) );
    resetForm();
  }
  ...
  const handleConfirmSaveLearnedConcepts = ( data ) => {
    ...
    confirmDialog({
        ...
        accept: () => handleSubmitLearnedConceptsAbp( data ),
    });
  }
  ...
  useEffect(() => {
    if ( infoMsg.current?.state.messages?.length === 0 ) {
        infoMsg.current.show({
          severity: 'warn',
          detail: 'Solo el moderador puede ingresar los conceptos que el equipo conoce, ' +
      ...
      <div className='grid p-fluid'>
        <div className='col-10'>
          <h5>
            ...
            Estos son los conceptos que el equipo conoce...
          </h5>
        </div>
        <div className='col-2'>
          <Button 
